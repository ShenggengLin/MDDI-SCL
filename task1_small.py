...
def cross_val(feature,label,event_num,each_event_num):
...
def main():
    ...
    start=time.time()
    result_all, result_eve=cross_val(new_feature,new_label,event_num,each_event_num)
...
def save_result(filepath,result_type,result):
    with open(filepath+result_type +'task1_small'+ '.csv', "w", newline='',encoding='utf-8') as csvfile:
